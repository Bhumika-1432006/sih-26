...
| Batched restarted PDHG for safe branch-and-bound node bounding and strong-branching scoring (#520; CPU reference - K nodes share one constraint matrix and one scaling, differing only in their column box, so the per-node loop is what a GPU port replaces with one SpMM per iteration across an n x K / m x K block; no CUDA kernel here, no CUDA toolkit in this environment) | Chambolle & Pock (2011) and Applegate et al. (PDLP, 2021), as above, for the iteration and step size; arXiv:2601.21990 for batched first-order strong branching on GPU, the idea this issue and this file are scoped from | `src/pdhg/pdhg_batch.cpp` |
