...
| PDHG-projection feasibility pump (#509; `pdhg_feasibility_pump`, off by default): the existing feasibility pump's L1-projection LP, solved through the restarted PDHG instead of whatever `algorithm` would otherwise pick, round by round; PDHG has no warm start from a previous iterate yet, so each round is a fresh solve | Fischetti, Glover & Lodi, *The feasibility pump*, Math. Programming 104 (2005); Corduk, Anjos & Vannelli, arXiv:2510.20499, for pairing a feasibility pump with a first-order LP engine | `src/mip/heuristics.cpp` (`feasibility_pump`, `PumpEngine::kPdhg`) |
| Fix-and-propagate (#509; `mip_heur_fix_and_propagate`, auto follows `mip_heuristics`): a root PDHG relaxation's point sorts the integer columns by fractionality least-fractional first, each fixed in turn with row-activity bound propagation after every fix, a bounded number of one-level backtracks on a propagated infeasibility, and `repair()` closing whatever rows are still violated as the temporary stand-in for Feasibility Jump (#506, not built on this branch) | Mexi, Besançon, Bolusani, Chmiela, Muñoz & Hendel, *Scylla: a matrix-free fix-propagate-and-project heuristic for mixed-integer optimization*, arXiv:2307.03466; Corduk, Anjos & Vannelli, arXiv:2510.20499; the propagation step is Savelsbergh (1994), above, reimplemented as a pure function since presolve's own version is private to its internal Workspace | `src/mip/heuristics.cpp` (`propagate_bounds`, `fix_and_propagate`), `src/mip/branch_and_bound_heuristics.cpp` (`run_fix_and_propagate`) |
