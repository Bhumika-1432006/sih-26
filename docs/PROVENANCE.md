...
| Whole-matrix activity-based domain propagation to a fixpoint (#510; `whole_matrix_propagation`, off by default): one pass computes every row's min/max activity from the current column bounds and reduces the bound each row implies for a column into a per-column candidate with a running min/max, Jacobi style across the whole matrix; passes repeat until nothing tightens. This is the CPU reference the paper's GPU kernel (one thread per row, atomics for the column reduction) is not implemented here to match - no CUDA hardware in this environment - and is wired once, at the branch-and-bound root, before the tree opens | Sofranac, Gleixner & Pokutta, *Accelerated domain propagation for mixed-integer linear programs on GPUs*, arXiv:2009.07785 (2020); the row/column activity formula itself is Brearley, Mitra & Williams (1975) and Savelsbergh, ORSA J. Computing 6(4) (1994), as already cited above for presolve and node propagation | `src/presolve/domain_propagation.cpp` |
